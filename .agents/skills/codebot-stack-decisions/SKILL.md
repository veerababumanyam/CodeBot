...
|-----------|-------------|
| Aider / Continue CLI | Codex + Codex + Gemini CLI cover v1 needs |
...
|-------|------------|----------|----------------|
| Codex | Agent SDK (in-process) | S5 Implementation, S8 Debug | Built-in tools, subagents, MCP, session resume |
...
- **Path 1 — LLM Gateway (API reasoning):** All API-based providers are accessed through
  ...
  LLM spend by 30-50%.
- **Path 2 — Direct CLI Agent Integration (autonomous coding):** Codex (Agent SDK,
...
| Codex / Codex / Gemini CLI through LiteLLM? | No — CLI agents integrate directly, bypassing the gateway |
