...
description: >
  ...
  CLI Agent Bridge (Claude Code SDK, Codex, Gemini), DuckDB cost analytics, token budgets,
...
CodeBot Agent
    ...
    └── PATH 2: Direct CLI Agent Integration (autonomous coding)
        Agent -> Claude Code Agent SDK    (in-process, full agent)
...
- Claude Code, Codex CLI, and Gemini CLI are integrated **directly**, not through LiteLLM
...
apps/server/src/codebot/cli_agents/
  ...
  claude_code.py     # Claude Code agent (SDK integration)
...
|-----------|---------|----------|-------------|
| Claude Code | `claude-agent-sdk` | Complex coding, multi-file refactoring, debugging | SDK (in-process) |
...
connections. This is the **alternative to the Claude API** for tasks that need
autonomous file editing, shell execution, and codebase exploration.

```
CodeBot Pipeline Agent (e.g., BackendDevAgent in S5)
    │
    ├── Reasoning path: LiteLLM Proxy → Claude/OpenAI/Google API
    │   (planning, analysis, review — agents that think)
    │
    └── Coding path: CLI Agent Bridge → Claude Code / Codex / Gemini CLI
...
### 4a. Claude Code Integration (Agent SDK)
...
Claude Code is integrated via the `claude-agent-sdk` Python package, which
communicates with the Claude Code CLI under the hood. It provides built-in
tools (Read, Write, Edit, Bash, Glob, Grep), permission controls, subagent
spawning, and MCP server support.

```python
# apps/server/src/codebot/cli_agents/claude_code.py
...
class ClaudeCodeAgent(CLIAgentRunner):
    """Claude Code as an autonomous coding agent via Agent SDK."""
    ...
    async def send_prompt(self, session_id: str, prompt: str) -> str:
        ...
        try:
            ...
            raise RuntimeError(f"Claude Code connection failed: {e}")
...
**Exposing CodeBot tools to Claude Code via MCP:**
...
STAGE_DEFAULTS = {
    ...
    "S5_implementation": "claude_code",  # Best autonomous coding
    "S5_tests": "codex",           # Strong test generation
    "S7_testing": "codex",         # Test execution and fixing
    "S8_debug": "claude_code",     # Complex debugging
}
```
...
3. Choose integration: SDK (like Claude Code) or subprocess (like Codex/Gemini)
...
| Task | Where |
|------|-------|
| Add Claude Code agent | `cli_agents/claude_code.py` (Agent SDK) |
...
LiteLLM and Claude Agent SDK APIs evolve rapidly. Always check Context7 for current method signatures.
